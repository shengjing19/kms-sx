...
#if _MSC_VER >= 1200
#pragma warning(push)
#pragma warning(disable:4201) // C4201:使用了非标准扩展：无名称结构/联合
#endif
...
struct IUnknown; // “combaseapi.h（229）：错误C2760:语法错误：使用/permission时意外的标记“identifier”，应为“type说明符”的解决方法-
...
// 伪造的KMS主机IP地址
#define LOCALHOST_IP L"10.0.0.1"
...
// Hook functions prototype   挂钩功能原型
...
// Original function pointers    原始函数指针
...
RPC_STATUS RPC_ENTRY RpcStringBindingComposeW_Hook(WCHAR *ObjUuid, WCHAR *ProtSeq, WCHAR *NetworkAddr, WCHAR *EndPoint, WCHAR *Options, WCHAR **StringBinding)
{
	...
	// Check destination address and hook   检查目的地地址和挂钩
	if (ProtSeq != nullptr && _wcsicmp(ProtSeq, PROTO_SEQ_TCP) == 0)
	{
		// Redirect rpcrt4 call to localhost   将rpcrt4调用重定向到localhost
...
// Create RpcStringBindingComposeW and GetProcAddressHook hooks     创建RpcStringBindingComposeW和GetProcAddressHook挂钩
BOOL WINAPI PatchIATInternal(HMODULE hModule, APIHook* APIHookInfo)
{
	// Get original function addresses being hooked   获取被钩住的原始函数地址
	FARPROC Original = GetProcAddress(GetModuleHandleA(APIHookInfo->module), APIHookInfo->name);
	if (Original == nullptr)
		return FALSE;

	// Hold original address  保留原始地址
	if (APIHookInfo->original == nullptr)
		APIHookInfo->original = (void*)Original;

	// Get base address of our process primary module  获取我们的流程主模块的基本地址
